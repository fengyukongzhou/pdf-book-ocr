...
**PDF Book OCR** 结合了“本地 0-Token 确定性探针 + 微型切片隔离并发转写 + 确定性代码断缝质检与章节脚注隔离 + 出版级 EPUB 3 双向气泡增强”，彻底根治上述所有顽疾。
...
[原始 PDF 图书 (100~500+页)]
 ...
 digitize_book.py ──> 提取 300 DPI 超清封面、检文字层、生成切片任务单 (subagent_jobs.json)
       │
   ┌───┴───────────────────────────────────────┐
   │ [数字文字版 PDF (检索率 > 75%)]           │ [无文字扫描版 PDF]
   ▼                                           ▼ (Layer 1: 物理微切片)
本地无损直提 (0 Token 秒级出书)            pdf_slicer.py ──> 10~15 页独立微型 PDF (parts/)
   │                                           │
   │                                           ▼ (Layer 2: 隔离上下文并发视觉转写)
   ...
   │                                           │
   │                                           ▼ (Layer 3: 确定性代码合流与质检)
   │                                     seam_auditor.py ────> 逐字断缝核验，确保 0 丢句、0 重复
   │                                     chapter_assembler.py ─> 章节脚注命名空间隔离 ([^c01_1])
   │                                           │
   └───────────────────┬───────────────────────┘
                       ▼ (Layer 4: 出版级编译与格式净化)
  epub_builder.py ───> 调用 Pandoc 编译 EPUB 3 + 注入古典纸书 CSS + 激活双向气泡弹窗
       ...
       ┌───────────────┴───────────────┐
       ▼                               ▼
《书名》.epub (微信读书/Apple Books/多看/Kindle)  《书名》.md (Obsidian 典藏主笔记，完备 Frontmatter)
```
...
- ⚡ **0-Token 极速直提（双轨机制）**：自动探针检测。若 PDF 含有原生文字层，5 秒内本地提取并编译出书，**耗费 0 个视觉 Token**。
...
└── scripts/
    ├── digitize_book.py      # 一键主控 CLI（含 --doctor, --assemble, 0-Token 探针）
    ├── pdf_analyzer.py       # 0-Token 预检、300 DPI 封面提取与切分规划
